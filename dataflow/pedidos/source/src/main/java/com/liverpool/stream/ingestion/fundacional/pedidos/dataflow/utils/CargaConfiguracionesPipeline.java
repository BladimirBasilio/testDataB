...
public class CargaConfiguracionesPipeline  {
	...
	public RegistraPedidoOptions inicia() {
		String jdbcUrlSM;
		...
		//define pipeline options
		 options = PipelineOptionsFactory.create()
												.as(RegistraPedidoOptions.class);
		...
		try
		{
			...
			options.setAppName(config.getString("gcp.df.registra.pedido.appName"));
			LOG.info("La AppName es:" + config.getString("gcp.df.registra.pedido.appName"));
			...
			options.setJobName(config.getString("gcp.df.registra.pedido.appName")  + "dfjob");
			...
			options.setNombreTopico(config.getString("gcp.pubsub.topic.registra.pedido"));
			options.setNombreSuscripcion(config.getString("gcp.pubsub.topic.subscription.registra.pedido"));
			...
			options.setConsultaInsertPedidos(config.getString("jdbc.insert.query.pedidos.pedidos"));
			options.setConsultaInsertPedidosDetalleSku(config.getString("jdbc.insert.query.pedidos.pedidos.detalle.sku"));
			...
			options.setMdmUrl(config.getString("mym.url"));
			options.setMdmServicesEndpoint(config.getString("mym.services.endpoint"));
			options.setMdmServicesTimeOut(config.getInt("mym.services.timeout.connect"));

			options.setRedisHost(config.getString("spring.redis.host"));
			options.setRedisPort(config.getInt("spring.redis.port"));
			options.setRedisPassword(config.getString("spring.redis.password"));
			options.setRedisExpireKey(config.getInt("spring.redis.expire.key"));
			options.setRedisTimeout(config.getInt("spring.redis.timeout"));
			options.setJedisPoolMaxIdle(config.getInt("spring.redis.jedis.pool.max-idle"));
			options.setJedisPoolMinIdle(config.getInt("spring.redis.jedis.pool.min-idle"));
			options.setJedisPoolMaxActive(config.getInt("spring.redis.jedis.pool.max-active"));
			options.setJedisPoolMaxWait(config.getInt("spring.redis.jedis.pool.max-wait"));

			options.setVinculacionTopic(config.getString("gcp.pubsub.topic.boleta.vinculacion"));
			options.setVinculacionProjectId(projectId);
		}
